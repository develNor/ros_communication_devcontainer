...
def _load_session_config_from_host(session_host_dir: Path) -> dict[str, Any]:
    ...
    for name in candidates:
        ...
        if isinstance(param, dict) and "load_template" in param:
            session_template_fs, provided_params = session_gen._parse_session_config_template_spec(param, str(fp.parent))
    ...
    raise RuntimeError(
        ...
        f"{candidates}. Got dir: {session_host_dir}"
    )
...
def _base_extra_run_args(runtime: RuntimeConfig, session: ResolvedSession, cfg: dict[str, Any]) -> list[str]:
    ...
    if session.source == "workspace" and not ros2docker_cfg.get("mount_ws"):
...
def doctor(args: argparse.Namespace) -> int:
    ...
    if _ROS2DOCKER_IMPORT_ERROR:
        ...
        line(
            "OK",
            "ros2docker",
            f"{getattr(ros2docker, '__version__', 'unknown')} from {Path(ros2docker.__file__).resolve()}",
        )
    ...
    if runtime and _ROS2DOCKER_IMPORT_ERROR is None:
        try:
            ...
            if not config.get("mount_ws"):
                line("WARN", "mount_ws", "false; rosotacom will add /ws dynamically for repo sessions")
...
def smoke(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        plugin_text = "\n".join(
            (session.host_dir / peer / "plugin.yaml").read_text(encoding="utf-8")
            for peer in ("a", "b")
        )
        ...
        if not args.keep_running:
            runtime = _load_runtime_config(args)
            for container_name in [a_container, b_container]:
                if container_name:
                    _stop_container_name(container_name, runtime)
    return 0
